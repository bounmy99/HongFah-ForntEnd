...
import { GetAllWithDraw, GetWallet, GetOneWithDraw, ApprovedWithDraw, RejectWithDraw } from '../../functions/WithDraw';
...
const ListWithdraw = () => {
    ...
    const loadData = () => {
        GetWallet(users.token).then(res => {
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: row => (
                ...
                <>
                    {row.status === 'success' && <p style={{ color: '#00B488' }}>{row.status}</p>}

                    {row.status === 'await' && <p style={{ color: 'darksalmon' }}>{row.status}</p>}
                </>
        ...
        <div className="card-main">
            ...
            {loading ? <h1>ກຳລັງໂຫຼດຂໍ້ມູນ........</h1> : <>
                ...
                <div className="withdraw-table">
                    ...
                    <DataTables columns={columns} OrderList={withDraw} customStyles={customStyles} />
                </div>
            </>}
